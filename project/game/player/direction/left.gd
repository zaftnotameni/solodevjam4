class_name PlayerDirectionLeft extends Node2D
...
@onready var wall_in_front := %CastDetectWallLeft
...
func _physics_process(_delta: float) -> void:
	...
	if wall_in_front.is_colliding():
		machine_direction.transition(PlayerEnums.Direction.RIGHT, 'bumped-into-wall')
